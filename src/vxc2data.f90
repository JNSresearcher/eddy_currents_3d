...
INTEGER lst, neww, np, kp, ier, idev, ver_python
LOGICAL uncompress
...
numfun = 0;  numMech=0; numVenv=0; !size_PHYS_C=0;
...
!filling arrays
kp=0; ios=0; numfun=0; numMech=0; numVenv=0
idev = 1
convert_strings1: &
DO WHILE ( 1==1)
    ...
    IF ( k0 > 0 ) THEN 
        ...
        DO i=2,neww
            IF (index( words(i),'REG') /=0 .and. kp <= nsub) THEN
                ...
                DO j=i+1, neww-1
                    IF ( words(j)(1:1)==  'C') THEN 
                        ...
                        IF (valPHYS(kp,2) /=0.d0) THEN
                            ...
                            numdom_C = kp
                            valdom_C = 2.d0*valPHYS(kp,2)/dt ! double for force calc dinamics
            ...
            ! for ENVIRON calculate  C etc
                DO j=i+1, neww-1
                    IF     (  words(j)(1:1)=='D' ) THEN 
                        valPHYS(nsub_glob,1) = evaluate(words(j + 1))
                    ELSEIF (  words(j)(1:1)=='C') THEN 
                        print*, 'environ with C not use !!!'
                        stop
                    ELSEIF ( index( words(j),'VEX'  )  /=0) THEN
                        print*, 'environ with VEX not use !!!'
                        stop
                    ELSEIF ( index( words(j),'VEY')  /=0) THEN 
                        print*, 'environ with VEY not use !!!'
                        stop
                    ELSEIF ( index( words(j),'VEZ')  /=0) THEN 
                        print*, 'environ with VEZ not use !!!'
                        stop
                    ENDIF
                ENDDO
...
IF (numdom_C /= 0) THEN
    ...
    do k=1,sdz; do j=1,sdy; do i=1,sdx
        n = n + 1; 
        if (v(n) == numdom_C) THEN
            m = m + 1
            geoPHYS_C(i,j,k) = 3*Cells + m  !local number
        endif
    enddo;enddo;enddo
    
! the global number of cell numbers for each numdom_C 
    siznod_C = count(v == numdom_C)
    ALLOCATE (nods_C(siznod_C), source=0   )
    k=0
    DO j=1,Cells
        IF (v(j) == numdom_C) THEN
            k=k+1
            nods_C(k) = j   ! number cell with C in global numbers
        ENDIF
    ENDDO
ENDIF
