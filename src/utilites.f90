...
SUBROUTINE writeVtk_field (Npoint,  sdx, sdy, sdz, nCells, delta, Uaf, Jaf,Jvx,Jvy,Jvz, geoPHYS_C, nods_C, files)
! This program generates a .vtk file for fields 
IMPLICIT NONE

INTEGER,      INTENT(IN):: Npoint,  sdx,sdy,sdz, nods_C
REAL(8),      INTENT(IN):: Uaf(*), Jaf(*), Jvx(*), Jvy(*), Jvz(*), delta(3)
CHARACTER(*), INTENT(IN):: files
...
!BUFFERED='YES', BLOCKSIZE=32777216,
...
IF (nods_C /= 0 ) THEN
    ...
    WRITE(n_new) "VECTORS "//"eddy_current"//" float"//new_line(ci)
    m = 0; s=-0.07957747154594766788444d7    ! 1/mu0         
    ! sxm=0.d0
    do k=1,sdz; do j=1,sdy; do i=1,sdx
        ...
        if ( geoPHYS_C(i,j,k) /=0 ) then
            ...
            write(n_new) 0.0, 0.0, 0.0
        endif
    enddo; enddo;enddo
    write(n_new)  new_line(ci)

    ! vector field output :  source
    WRITE(n_new) "VECTORS "//"field_SOURCE"//" float"//new_line(ci)
    m=0; !sxm=0.d0
    DO k = 1,sdz;  DO j = 1,sdy;  DO i = 1,sdx
        m = m+1
        n = geoPHYS_C(i,j,k)
    ...
    write(n_new) "VECTORS "//"eddy_current_summ"//" float"//new_line(ci)
    ! m=0
    s=-0.07957747154594766788444d7
    do k=1,sdz;   do j=1,sdy;   do i=1,sdx
        ! m=m+1
        IF ( geoPHYS_C(i,j,k) /= 0) THEN ! is domain with C
            n = geoPHYS_C(i,j,k) - 3*nCells
            write(n_new) real(s*Jvx(n),4), real(s*Jvy(n),4), real(s*Jvz(n),4)
        ELSE
            write(n_new) 0.0, 0.0, 0.0
        endif
    enddo;   enddo;  enddo
...
write(n_new) "VECTORS "//"field_B"//" float"//new_line(ci)
m=0
do k=1,sdz;  do j=1,sdy;  do i=1,sdx               
    m=m+1
