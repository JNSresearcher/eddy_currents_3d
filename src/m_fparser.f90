MODULE m_fparser
  ...
  ! and
  ! This realisation is based on the program on written by Roland
  ! Schmehl <roland.schmehl@alumni.uni-karlsruhe.de> available from http://fparser.sourceforge.net
  !
  !------- -------- --------- --------- --------- --------- --------- --------- -------

! Fortran code  modified by J.Sochor   ( https://github.com/JNSresearcher )
  ...
  TYPE tComp
     ...
     REAL(8),    DIMENSION(:), POINTER :: Immed
     INTEGER                            :: ImmedSize
     REAL(8),    DIMENSION(:), POINTER ::Stack
     INTEGER                            :: StackSize, &
                                           StackPtr
  END TYPE tComp
...
CONTAINS
  ...
  FUNCTION evalf (i, Val) RESULT (res)
    ...
    IMPLICIT NONE
    INTEGER,                INTENT(in) :: i                  ! Function identifier
    REAL(8),    DIMENSION(:), INTENT(in) :: Val       ! Variable values
    REAL(8):: res       ! Result
    INTEGER                            :: IP,              & ! Instruction pointer
                                          DP,              & ! Data pointer
                                          SP                 ! Stack pointer
    REAL(8),PARAMETER :: zero = 0.0D+00!(0.D+00,0.D+00)
    real(8):: yR!,yI
    !----- -------- --------- --------- --------- --------- --------- --------- -------
    DP = 1
    SP = 0
    DO IP=1,Comp(i)%ByteCodeSize
       ...
       CASE   (cDiv); IF (Comp(i)%Stack(SP)==zero) THEN; EvalErrType=1; res=zero; RETURN; ENDIF
       ...
       CASE (cLog10); IF (REAL(Comp(i)%Stack(SP),8)<=0._8) THEN; EvalErrType=3; res=zero; RETURN; ENDIF
                      Comp(i)%Stack(SP)=LOG10(REAL(Comp(i)%Stack(SP),8)); !CMPLX(yR,0._8)
       CASE   (cLog); Comp(i)%Stack(SP)=LOG(Comp(i)%Stack(SP))
       CASE  (cSqrt); Comp(i)%Stack(SP)=DSQRT(Comp(i)%Stack(SP))!ZSQRT(Comp(i)%Stack(SP))
       CASE  (cSinh);  yR=REAL(Comp(i)%Stack(SP),8); !yI=IMAG(Comp(i)%Stack(SP));
                       !IF ((yI<-1._8).OR.(yI>1._8)) THEN; EvalErrType=5; res=zero; RETURN; ENDIF
                       !Comp(i)%Stack(SP)=CMPLX( sinh(yR)*cos(yI), cosh(yR)*sin(yI))
                        Comp(i)%Stack(SP)=sinh(yR)
       CASE  (cCosh); yR=REAL(Comp(i)%Stack(SP),8); !yI=IMAG(Comp(i)%Stack(SP));
                       !IF ((yI<-1._8).OR.(yI>1._8)) THEN; EvalErrType=5; res=zero; RETURN; ENDIF
                       !Comp(i)%Stack(SP)=CMPLX( cosh(yR)*cos(yI), sinh(yR)*sin(yI))
                       Comp(i)%Stack(SP)=cosh(yR)
       CASE  (cTanh); yR=REAL(Comp(i)%Stack(SP),8);! yI=IMAG(Comp(i)%Stack(SP));
                    !IF ((yI<-0.5_8).OR.(yI>0.5_8)) THEN; EvalErrType=6; res=zero; RETURN; ENDIF
                    !Comp(i)%Stack(SP)=CMPLX( sinh(2.0_8*yR),sin(2.0_8*yI))/(cosh(2.0_8*yR)+cos(2.0_8*yI))
                    Comp(i)%Stack(SP)=sinh(yR)/cosh(yR)
       CASE  (cCtanh); yR=REAL(Comp(i)%Stack(SP),8); !yI=IMAG(Comp(i)%Stack(SP));
                    !IF ((yI<-0.5_8).OR.(yI>0.5_8)) THEN; EvalErrType=6; res=zero; RETURN; ENDIF
                    !Comp(i)%Stack(SP)=CMPLX(sinh(2.0_8*yR),-sin(2.0_8*yI))/(cosh(2.0_8*yR)-cos(2.0_8*yI))
                    Comp(i)%Stack(SP)=cosh(yR)/sinh(yR)
       ...
       CASE  (cAsin); IF ((REAL(Comp(i)%Stack(SP),8)<-1._8).OR.(REAL(Comp(i)%Stack(SP),8)>1._8)) THEN
                      EvalErrType=4; res=zero; RETURN; ENDIF
                      Comp(i)%Stack(SP)=DASIN(REAL(Comp(i)%Stack(SP),8))!CMPLX(DASIN(REAL(Comp(i)%Stack(SP),8)),0._8)
       CASE  (cAcos); IF ((REAL(Comp(i)%Stack(SP),8)<-1._8).OR.(REAL(Comp(i)%Stack(SP),8)>1._8)) THEN
                      EvalErrType=4; res=zero; RETURN; ENDIF
                      Comp(i)%Stack(SP)=DACOS(REAL(Comp(i)%Stack(SP),8))
                      
       CASE  (cSind); Comp(i)%Stack(SP)=sind(Comp(i)%Stack(SP))
       CASE  (cCosd); Comp(i)%Stack(SP)=cosd(Comp(i)%Stack(SP))
       CASE  (cTand); Comp(i)%Stack(SP)=TAND(Comp(i)%Stack(SP))
                      
       CASE  (cU);    if ( Comp(i)%Stack(SP) > 0._8 ) then
                           Comp(i)%Stack(SP) = 1.0_8;  else; Comp(i)%Stack(SP) = 0.0_8; endif
                           
       CASE  (cU2);    if ( Comp(i)%Stack(SP) >= 0._8 ) then
                           Comp(i)%Stack(SP) = 1.0_8;  else; Comp(i)%Stack(SP) = -1.0_8; endif

       CASE  (cPos);  if ( Comp(i)%Stack(SP) > 0._8 ) then
                           Comp(i)%Stack(SP) = Comp(i)%Stack(SP);  else; Comp(i)%Stack(SP) = 0.0_8; endif
       CASE  (cInt); Comp(i)%Stack(SP)= aint(Comp(i)%Stack(SP))
       CASE  (cNInt); Comp(i)%Stack(SP)= anint(Comp(i)%Stack(SP))
       CASE  (cFloor); Comp(i)%Stack(SP)= floor(Comp(i)%Stack(SP))
       CASE  (cCeil); Comp(i)%Stack(SP)= ceiling(Comp(i)%Stack(SP))
       CASE  (cAtan); Comp(i)%Stack(SP)=DATAN(REAL(Comp(i)%Stack(SP),8))
       !CASE  (cRe);   yR=REAL(Comp(i)%Stack(SP),8);    Comp(i)%Stack(SP)= yR !CMPLX(yR,0._8)
       !CASE  (cIm);   yR=DIMAG(Comp(i)%Stack(SP));      Comp(i)%Stack(SP)= yR !CMPLX(0._8,yR)
  ...
  FUNCTION EvalErrMsg () RESULT (msg)
    ...
    CHARACTER (LEN=*), DIMENSION(6), PARAMETER :: m = (/ 'Division by zero                ', &
                                                         ...
                                                         'Imag part of sh or ch illegal   ', &
                                                         'Imag part of th or cth illegal  '/)
  ...
  FUNCTION MathFunctionIndex (str) RESULT (n)
    ...
    DO j=cAbs,cAtan                                          ! Check all math functions
       ...
       ! k = MIN(LEN_TRIM(Funcs(j)), LEN_trim(str))
